...
void MapUI::PrintMenu() {
  ...
  case 4:
  {
    ...
    if (output.empty()){
      std::cout << "No locations found for this category.\n";
    } 
    else{
      std::cout << "Matching Location IDs:\n";
  ...
  case 5:
  {
    ...
    // fill in here
    std::string pattern;
    std::cout << "Please enter a regular expression pattern: ";
    std::getline(std::cin, pattern);  //get input

    try {
      std::regex regex(pattern);
      std::vector<std::string> results = map.GetLocationRegex(regex);

      if (results.empty()) {
        std::cout << "No locations match the regular expression.\n";
      } else {
        std::cout << "Matching Location IDs:\n";
